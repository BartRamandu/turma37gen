...
public class C5Bank {
	public static void main(String[] args) {
		...
		do {
					...
					if (opMovim == 'D' || opMovim == 'L') {
						...
						while (true) {
							...
							if (valor > (conta.getSaldo() + conta.getLimite())) {
								System.out.println("\nSeu saldo é insuficiente.");
								System.out.print("Digite um valor menor que R$" + conta.getSaldo()
										+ " e que esteja dentro do limite atual de R$" + conta.getLimite() + ": R$ ");
								valor = sc.nextDouble();
			...
			System.out.println("");
			System.out.print("\nDeseja realizar novamente uma operação com Contas, [S] - Sim ou [N] - Não: ");
			cont = sc.next().toUpperCase().charAt(0);
			limpa();
		}while(cont=='S');
		
		nome();
		System.out.println("\nAgradeçemos a preferência, esperamos que tenha tido uma ótima estadia, até mais e volte sempre!");
	}
